 ...
 * 3) mark the <pre> tags in your source with class="code". <code> tags are
 ...
 *
 * Change log:
 * Andreas Blixt <andreas@blixt.org>, 2008/05/30
 *   Changed class name to 'code' rather than 'prettyprint' for <pre> and
 *   removed class requirement from <code> tags to separate style from HTML
 *   structure, and removed the requirement to add onload attribute to the
 *   <body> tag for the same reason.
 * cbeust, 2006/08/22
...
/** find all <code> tags and all the <pre> tags in the DOM with class="code" and
  * prettify them.
...
function prettyPrint(opt_whenDone) {
  ...
  function doWork() {
    ...
    for (; k < elements.length && new Date().getTime() < endTime; k++) {
      var cs = elements[k];
      if (cs.nodeName == 'CODE' || (cs.className && cs.className.match(/\bcode\b/))) {
        ...
        for (var p = cs.parentNode; p != null; p = p.parentNode) {
          if (((p.tagName == 'pre' ||
               p.tagName == 'xmp') &&
              p.className && p.className.match(/\bcode\b/)) || p.tagName == 'code') {
            nested = true;
            break;
          }
        }
...
// Original code for checking DOM readiness from
// http://www.javascriptkit.com/dhtmltutors/domready.shtml
// Modified by Andreas Blixt <andreas@blixt.org>
var domReady = (function () {
    ...
    return function (callback) {
        ...
        if (/safari/i.test(navigator.userAgent)) {
            ...
            document.write('<script type="text/javascript" id="domready__" defer="defer" src="javascript:void(0)"><\/script>');
            document.getElementById('domready__').onreadystatechange = function () {
        ...
        window.onload = function () {
            setTimeout(function () { if (callbacks) go(); }, 0);
        };
    }
})();
